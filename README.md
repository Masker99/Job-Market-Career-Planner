...
```text
outputs/v5_20260510_001530_career_plan.md
```

脱敏后的 GitHub 版本默认保存为类似：

```text
reports/v5_20260510_001530_career_plan.md
```
...
```text
outputs/v5_20260510_001530_retrieval_debug.md
reports/v5_20260510_001530_retrieval_debug.md
```
...
- Vector Search：embedding provider、索引路径、向量维度和相似度
...
```powershell
python main.py --profile "熟悉 Python、RAG 和 FastAPI" --target-role "AI 应用工程师" --debug --report-version v5
```
...
| `JMCP_RETRIEVAL_MODE` | 检索模式，`vector` 使用 V5 向量检索，`keyword` 回退关键词检索 |
| `JMCP_EMBEDDING_PROVIDER` | Embedding 提供方，当前 `hash` 为零依赖学习版 |
| `JMCP_EMBEDDING_DIMENSION` | 本地 embedding 向量维度 |
| `JMCP_VECTOR_INDEX_PATH` | 本地向量索引 JSONL 路径 |
...
│   ├── vector_store.py  # Simple Vector Search
│   ├── retriever.py     # 关键词检索
...
V5：Embedding + Simple Vector Search RAG

在 V4 LLM Query Expansion RAG 的基础上，V5 增加本地 embedding 和 Simple Vector Search。系统会先把岗位文本和检索 query 转成向量，再用余弦相似度召回语义相近的岗位，然后继续生成市场画像和代表性岗位证据。这个版本重点学习 RAG 里的 Embedding、Vector Search、Cosine Similarity 和 Indexing。
...
- 使用本地 hash embedding 将岗位文本和检索 query 转成向量
- 使用 cosine similarity 进行 Simple Vector Search
- 将向量召回结果用于市场画像和代表性岗位证据
...
### V5 学习重点：Embedding 与 Vector Search

V1/V2 的重点是 Retrieval：如何把相关岗位找出来。V3 的重点是 Context Engineering：如何把找出来的一批岗位压缩成更有用的上下文。V4 的重点是 Query Transformation：先把用户目标改写成更贴近岗位市场的检索语言。V5 的重点是 Vector Search：文本不再只靠字面词匹配，而是转成向量后比较相似度。
...
V5 当前使用 `hash` embedding，这是零依赖、可复现的学习版 embedding，用来帮助理解向量索引、向量维度和相似度计算。它不是生产级语义模型；后续可以把 embedding provider 替换为真实 embedding API 或本地 embedding 模型。
...
- 接入真实 Embedding API 或本地 Embedding 模型
- 增加向量数据库 Chroma/FAISS
