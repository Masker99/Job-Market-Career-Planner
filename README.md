...
其中“简历文件”和“补充个人信息”至少需要填写一个。默认输出会保存到 `reports/`，文件名带有版本号和时间戳，便于提交到 GitHub 作为迭代记录。
...
```text
reports/v3_20260510_001530_career_plan.md
```
...
```text
reports/v3_20260510_001530_retrieval_debug.md
```
...
```powershell
python main.py --profile "熟悉 Python、RAG 和 FastAPI" --target-role "AI 应用工程师" --debug --report-version v3
```

建议把 `reports/` 中有代表性的运行结果提交到 GitHub，用来记录每个版本的输出质量、检索证据和调试过程。`outputs/` 仍然适合作为本地临时输出目录，默认不会提交。
